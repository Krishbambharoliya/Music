...
export default function PlayerClient() {
  ...
  return (
    <>
      ...
      {/* 90s Cassette Centerpiece (Absolute centered, responsive scaling) */}
      <div 
        className="absolute top-[40%] sm:top-[43%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-[240px] sm:w-[320px] aspect-[1.58/1] flex flex-col justify-between p-2.5 sm:p-3.5 rounded-xl border-[4px] sm:border-[5px] border-stone-800 bg-gradient-to-br from-stone-900/95 to-stone-950/98 shadow-[0_30px_70px_rgba(0,0,0,0.95),inset_0_1px_2.5px_rgba(255,255,255,0.15)] select-none pointer-events-none transition-all duration-500 ease-out"
        style={{
          boxShadow: isPlaying && currentTrack.id !== 'no-track'
            ? '0 30px 70px rgba(0,0,0,0.95), 0 0 15px rgba(224,161,58,0.15), inset 0 1px 2.5px rgba(255,255,255,0.15)'
            : '0 25px 50px rgba(0,0,0,0.85), inset 0 1px 2.5px rgba(255,255,255,0.1)'
        }}
      >
        {/* Cassette Top Write-on Line */}
        <div className="w-full flex justify-between items-center px-1 sm:px-1.5 text-[5px] sm:text-[6px] text-stone-500/80 font-bold uppercase tracking-widest font-retro mb-1">
        ...
        {/* Paper label sticker */}
        <div 
          className="w-full flex-1 rounded-lg border-2 border-stone-900/70 p-2 sm:p-3 flex flex-col justify-between relative shadow-md overflow-hidden bg-[#e8e0cd]"
          style={{
            backgroundImage: 'radial-gradient(circle at 50% 50%, #eae2cf 60%, #ded5be 100%)'
          }}
        >
          ...
          {/* Center Transparent Glass Window */}
          <div className="w-[64%] h-[36%] bg-[#080b11]/95 rounded-md border-2 border-stone-900/90 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-between px-3 sm:px-4 overflow-hidden shadow-[inset_0_2px_8px_rgba(0,0,0,0.85)] z-20">
            {/* Left tape reel */}
            <div className="relative flex items-center justify-center">
              {/* Tape Roll wrap (size decreases as progress increases) */}
              <div 
                className="absolute rounded-full bg-gradient-to-r from-[#291b0f] to-[#120b05] border border-stone-950 transition-all duration-500 shadow-inner"
                style={{
                  width: `${26 + (1 - progressPct / 100) * 16}px`,
                  height: `${26 + (1 - progressPct / 100) * 16}px`
                }}
              />
              {/* Gear */}
              <svg 
                className={`w-5 h-5 text-stone-400 z-10 ${isPlaying && currentTrack.id !== 'no-track' ? 'reel-rotate' : ''}`}
                style={{
                  animationPlayState: isPlaying && currentTrack.id !== 'no-track' ? 'running' : 'paused'
                }}
                viewBox="0 0 24 24" 
                fill="none" 
                stroke="currentColor" 
              ...
              >
                ...
                <path d="M12 2v4M12 18v4M4 12H2M22 12h-4M5.6 5.6l2.8 2.8M15.6 15.6l2.8 2.8M18.4 5.6l-2.8 2.8M8.4 15.6l-2.8 2.8"/>
              </svg>
            </div>

            {/* Middle see-through glass text indicating tape length */}
            <span className="text-[5.5px] sm:text-[6.5px] text-[#ebb548]/35 font-retro font-bold select-none leading-none z-10 tracking-widest">
              {currentTrack.id === 'no-track' ? 'C-00' : 'C-60'}
            </span>

            {/* Right tape reel */}
            <div className="relative flex items-center justify-center">
              {/* Tape Roll wrap (size increases as progress increases) */}
              <div 
                className="absolute rounded-full bg-gradient-to-r from-[#291b0f] to-[#120b05] border border-stone-950 transition-all duration-500 shadow-inner"
                style={{
                  width: `${26 + (progressPct / 100) * 16}px`,
                  height: `${26 + (progressPct / 100) * 16}px`
                }}
              />
              {/* Gear */}
              <svg 
        ...
        {/* Cassette bottom shape cut-outs */}
        <div className="w-full flex justify-between items-center px-6 sm:px-8 text-stone-600/70 font-retro text-[4.5px] sm:text-[5.5px] font-bold mt-1 leading-none select-none">
          <span>▲ A-SIDE</span>
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              <div className="flex-1 h-8 sm:h-11 select-none relative">
                <div
                  ...
                  className={`h-full w-full relative rounded-md border border-[#50370d] overflow-hidden cursor-pointer touch-none shadow-inner transition-all duration-500 ${
                    isPlaying && currentTrack.id !== 'no-track' ? 'dial-active-glow' : 'bg-[#070b13]'
                  }`}
                >
